% \version "2.24.0"
%
% Global.ily — shared definitions for one quartet.
% Edit ONLY the metadata block below for each new quartet; leave the
% functions / paper section alone (they are identical across all quartets).

%%% METADATA — EDIT THESE FOR EACH QUARTET =================================
cTitre = "Quatuor Op.?? n°? (G.???)"          % e.g. "Quatuor Op.39 (G.213)"
cCompositeur =	"Luigi BOCCHERINI"
cDates = "(1743-1805)"
cCompositeurDates = \markup {\center-column {\cCompositeur \cDates}}
cSource = \markup {\fontsize #-2 "Source: <publisher/library, place, year> (manuscript)"}
cRefConcert = "00000-0"                        % internal catalogue ref; optional
cCopyright = \markup {\abs-fontsize #5 \concat {
				\epsfile #X #2  #"cc.eps"  " 2012-" \italic {"Le Concert - "} \cRefConcert " / " \cCompositeur " - " \cTitre " - v.1" }}

%%% CONSTANTES ==============================================================
% Movement title markups, referenced by the part .ly files as \titreI etc.
% Add \titreIV if the quartet has four movements.
titreI = \markup {\bold \huge "I."}
titreII = \markup {\bold \huge "II."}
titreIII = \markup {\bold \huge "III."}
titreIV = \markup {\bold \huge "IV."}

% Compress stretches of rest in the separate parts into multi-measure rests.
silencesMulti = { \compressEmptyMeasures
									\override MultiMeasureRest.expand-limit = #1
									\set restNumberThreshold = #0 }

% Common Boccherini text/dynamic abbreviations. Use as e.g.  c4\rf   or   c4\dolce
...
%%% ondul: TextSpan en trait ondulé (wavy text-spanner, used for long trills etc.)
...
\paper {
    ...
    bottom-margin = 1\mm
		...
		markup-system-spacing = #'((basic-distance . 5)(padding . 3) (minimum-distance . 3)  )
		markup-markup-spacing = #'((basic-distance . 5)(padding . 3) (minimum-distance . 3)  )
		...
		score-system-spacing = #'((basic-distance . 12) (padding . 5) (minimum-distance . 5) )
