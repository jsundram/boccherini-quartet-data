\version "2.24.0"
\include "Global.ily"

% Separate part for Violoncelle. One \score block per movement. \silencesMulti
% collapses runs of rest into multi-measure rests. Each movement pulls the
% skeleton (Trame.ily, tag #'mvtN) and this instrument's music (tag #'partie).

#(set-global-staff-size 20)
#(ly:set-option 'point-and-click #f)

cInstrument = "Violoncelle"

\paper {
    first-page-number = 2
}	%% fin paper

\layout {		%% wavy text-spanner support (see Global.ily)
  \context {
    \Score
    \override TextSpanner.stencil = #draw-text-spanner-allow-squiggle
    \override TextSpanner.squiggle-length = 0.8
    \override TextSpanner.angularity = 0.8
    \override TextSpanner.thickness = 0.4
    \override TextSpanner.height = 0.3
    \override TextSpanner.orientation = 1
  }
}

\book {
\header {
    title = \cTitre
    composer =	\cCompositeurDates
		poet = \markup \box { \pad-around #1 { \cInstrument }}
		copyright = \cCopyright
		instrumentHeader = 	\markup { \cInstrument }
}	%% fin header

\score {		%%1er mvt
	\new Staff \with {instrumentName = \titreI}
	{ \silencesMulti
		<<	\keepWithTag #'mvtI {\include "Trame.ily"}
				\keepWithTag #'partie {\include "04-Vc-1.ily"}
		>>
	}
}	%% fin score 1er mvt
\pageBreak
\markup " "
\pageBreak
\score {		%%2e mvt
	\new Staff \with {instrumentName = \titreII}
	{ \silencesMulti
		<<	\keepWithTag #'mvtII {\include "Trame.ily"}
				\keepWithTag #'partie \include "04-Vc-2.ily"
		>>
	}
} %%	 fin score 2e mvt
\pageBreak
\score {		%%3e mvt
	\new Staff \with {instrumentName = \titreIII}
	{ \silencesMulti
		<<	\keepWithTag #'mvtIII {\include "Trame.ily"}
				\keepWithTag #'partie \include "04-Vc-3.ily"
		>>
	}
} %%	 fin score 3e mvt

}	%% fin book
